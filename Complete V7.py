...
def speech_to_text_en(audio_path, logger=None):
    ...
    temp_wav = os.path.join(tempfile.gettempdir(), "temp_stt.wav")
    pipe = None
    try:
        subprocess.run([
...
def text_translation(long_text, logger=None, progress_callback=None):
    ...
    try:
        ...
        for i, chunk in enumerate(text_chunks):
            ...
            # --- 🔧 FIX: ปรับ Prompt ให้ชัดเจนขึ้นสำหรับโมเดล 4B ---
            # 1. ย้ายคำสั่งมาไว้ใน User message เพราะโมเดลเล็กมักเมิน System role
            # 2. ใช้ Pattern: [คำสั่ง] -> [เนื้อหา] -> [คำตอบ]
            
            prompt_template = (
                "Translate the following English text into natural, informal spoken Thai. "
                "Keep technical terms (AI, Python, Code) in English.\n\n"
                "English Text:\n"
                "{text}\n\n"
                "Thai Translation:\n"
            )
            
            user_content = prompt_template.format(text=chunk)

            messages = [
                # ตัด System role ออก และรวมคำสั่งไว้ใน User แทน เพื่อบังคับให้โมเดลโฟกัส
            ...
            with torch.no_grad():
                outputs = model.generate(
                    ...
                    max_new_tokens=1024, 
                    do_sample=True, 
                    temperature=0.3, # Temperature ต่ำเพื่อให้แปลตรงไปตรงมา
                    top_p=0.9, 
                    repetition_penalty=1.1, # ลด penalty ลงเล็กน้อย (จาก 1.15) เพื่อให้ภาษาไหลลื่นขึ้น
                    pad_token_id=tokenizer.eos_token_id
                )
            
            translated_text = tokenizer.decode(outputs[0][len(input_ids[0]):], skip_special_tokens=True)
            
            # Clean up: บางครั้งโมเดลอาจเผลอใส่คำว่า "Thai Translation:" ติดมาด้วย ให้ลบออก
            translated_text = translated_text.replace("Thai Translation:", "").strip()
...
def text_to_speech_TH(text, output_path, gender="Female", logger=None, progress_callback=None):
    ...
    async def _gen_sequence():
        ...
        for i, chunk in enumerate(chunks):
            if not chunk.strip(): continue
            fname = os.path.join(tempfile.gettempdir(), f"temp_tts_{i}.mp3")
...
class ModernApp(ctk.CTk):
    ...
    def run_pipeline(self, gender):
        try:
            audio_temp_path = os.path.join(tempfile.gettempdir(), "temp_dubbing_audio.wav")

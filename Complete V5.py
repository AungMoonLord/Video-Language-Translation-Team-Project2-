...
import socket
import winsound 
import tempfile # ✅ 1. เพิ่ม Library นี้สำหรับจัดการไฟล์ชั่วคราว
...
def speech_to_text_en(audio_path, logger=None):
    ...
    # ✅ 2. แก้ path ให้ไปใช้ Temp Folder ของ Windows
...
def text_to_speech_TH(text, output_path, gender="Female", logger=None, progress_callback=None):
    ...
    async def _gen_sequence():
        ...
        for i, chunk in enumerate(chunks):
            if not chunk.strip(): continue
            # ✅ 3. แก้ path ให้ไปใช้ Temp Folder
...
class ModernApp(ctk.CTk):
    ...
    def run_pipeline(self, gender):
        try:
            # ✅ 4. แก้ path ไฟล์เสียงรวมให้ไปใช้ Temp Folder
